...
@interface JTCalendarWeekViewCell (){
    NSMutableArray *_daysViews;
}

@end

@implementation JTCalendarWeekViewCell
...
- (void)createDayViews
{
    ...
    if(!_daysViews){
        ...
        //NSLog(@"self.frame.size.width: %@",  NSStringFromCGRect(self.frame));
...
- (void)layoutSubviews
{
    ...
    //NSLog(@"subviews self.frame.size.width: %@",  NSStringFromCGRect(self.frame));
    
    
    CGFloat x = 0;
    CGFloat dayWidth = self.superview.frame.size.width / NUMBER_OF_DAY_BY_WEEK;
    CGFloat dayHeight = self.superview.frame.size.height;
